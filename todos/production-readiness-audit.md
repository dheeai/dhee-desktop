...
**Date:** 2026-05-15
**Scope:** dhee-desktop (renderer + main) + dhee-core (embedded engine)
...
- **Where:** `dhee-core/src/tasks/video/workflow/activeProject.ts:26` (`activeProjectDir` is a process-global). `dhee-core/src/server/runners/executorRunner.ts:7-10` explicitly documents that JobManager serializes per-project only — cross-project is the caller's responsibility.
...
- **Where:** `dhee-core/src/services/comfyui/ComfyUIClient.ts:45-63` (`enrichFetchError()` logs to `debug.log` only); `ComfyUIClient.ts:399` (HTTP errors caught upstream and dropped). LLM streaming JSON parse failures default to `{}` silently in `LLMClient.ts:791-789`. 200s timeout at `LLMClient.ts:490-492` is console-only.
...
- **Where:** `dhee-core/src/server/runners/BackgroundTaskRunner.ts:238-242` (cancel aborts the local controller). `dhee-core/src/services/comfyui/ComfyUIClient.ts:300-305` (`interrupt()` exists but is never called when cancel fires).
...
- **Where:** `dhee-desktop/src/main/settingsManager.ts:62-66` writes settings (including OpenRouter / OpenAI / Gemini / ComfyUI Cloud keys) to `~/Library/Application Support/dhee/dhee-settings.json` via electron-store. No encryption layer.
...
- **Where:** `dhee-core/src/core/planner/ExecutorAgent.ts` (~8 direct `writeFileSync` calls with no temp-file-and-rename). Stale `.executor.stop` sentinel is handled (`dhee-core/src/server/runners/backgroundTaskRunnerSingleton.ts:55-61`) but partial PNGs / half-written JSON / truncated MP4s are not.
...
- **Where:** Codebase has no `cost`, `token`, `pricing`, or `quota` tracking outside `accountManager.ts:40-77` (which fetches dhee Cloud credit balance but doesn't display it in-flow).
...
- **Fix:** (a) Show a running spend counter in the chat panel header (token sums from LLM responses, render counts from ComfyUI). (b) Surface AccountInfo.credits prominently. (c) Pre-run cost estimate before kicking off `dhee_run_to`. (d) Optional spend cap setting.
- **Severity:** HIGH
...
- **Symptom:** User has no idea what to type. The pi-orchestrator has rich tools (`dhee_run_to`, `dhee_status`, scene generation, override flows) that are completely undiscoverable. Compare to ChatGPT's example prompt cards on a fresh chat.
...
- **Where:** `dhee-core/src/services/comfyui/ComfyUIClient.ts:686-706` — WebSocket message handler relays `currentNode` to progress callback, but `dhee-core/src/services/providers/comfyui/ComfyUIProvider.ts:686-694` discards it. Desktop IPC has `tool_call` / `tool_result` / `notification` but no `step_progress` event.
...
- **Where:** `dhee-core/src/core/project/projectSchema.ts:117-118` defines `schemaVersion: 3`. `backfillProjectSchema.ts` exists as a CLI helper (`pnpm backfill-schema`). Desktop boot never calls it.
...
- **Symptom:** User hits a bug → emails "the app crashed" → support has nothing to investigate. `debug.log` lives at `~/Library/Application Support/dhee/logs/debug.log` and the user doesn't know it exists.
...
- **Fix:** Add an "Export project" menu that zips `project.json`, `assets/`, `prompts/`, `chapters/` into a `*.dhee-archive` file. Symmetric import.
- **Severity:** MEDIUM
...
- **FFmpeg/FFprobe path resolution** via `dhee_FFMPEG_PATH` / `dhee_FFPROBE_PATH` env vars — the correct packaged-app contract; already implemented (`dhee-core/src/core/timeline/ffmpegBinaries.ts`).
...
- **Test coverage** — 2348+ passing tests in dhee-core, comprehensive component tests for TimelinePanel and AssetRegenerateButton in the desktop.
