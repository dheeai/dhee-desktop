---
description: Restart the dhee-desktop Electron app — kill any running instance, rebuild the kshana-core dist (so source changes are picked up), then `npm start` in the background.
---

Run these steps in order. **Do NOT skip the dist rebuild** — the desktop loads kshana-core via package.json `exports` pointing to `./dist/...`, so source-only edits are invisible to the running app until tsup runs.
...
### 2. Rebuild the kshana-core dist

```bash
cd /Users/ganaraj/Projects/kshana-core && pnpm tsup
```
...
The desktop's webpack sometimes treats unchanged source as cache-clean even when `kshana-core` changed. Force a fresh main-process bundle:
...
- If `pnpm tsup` produces TypeScript diagnostic errors that come from `dhee-desktop` (e.g. `kshanaCoreManager.ts: Cannot find name '...'`), those are **expected noise** from the broader desktop tree — they don't affect the dist build. Only stop if tsup itself exits non-zero.
